...
import { formDataActions } from '../../redux/actions';
...
const addToFormData = (fieldName: string, fieldValue: string) => ({ type: formDataActions.ADD_TO_FORM_DATA, fieldName, fieldValue });
const updateAllFieldsOfFormData = (newState: any) => ({ type: formDataActions.UPDATE_ALL_FIELDS, newState });
const clearAllFields = () => {
  return ({ type: formDataActions.CLEAR_ALL_FIELDS });
};
...
const mapStateToProps: MapStateToPropsParam<any, any, any> = (state) => ({ formDataState: state.formDataState });

const DeliveryForm = Form.create<DeliveryFormProps>(
  { name: 'delivery_form' }
  )(connect(mapStateToProps, mapDispatchToProps)(withRouter((
    { langMap: lang, formDataState, addDataToFormData, clearFields, updateFieldsOfFormData, form: { getFieldDecorator, setFieldsValue, getFieldsValue }, form, history }: any) => {
  ...
  return (
    <div>
      <Form onSubmit={handleSubmit} onKeyDown={handleEscDown}  {...formItemLayout}>
        ...
        <Form.Item label={lang.customerNumber}>
        ...
        <Form.Item label={lang.phoneNumber}>
        ...
        <Form.Item label={lang.houseNumber}>
      ...
      <div style={{ display: 'flex', flexFlow: 'row nowrap', justifyContent: 'space-between' }}>
        <Button type="danger" size="large" onClick={handleClearClick}>{lang.clear} / ESC</Button>
        <Form.Item>
          <Button htmlType="submit" type="primary" size="large">{lang.goToOrder} / F2</Button>
        </Form.Item>
      </div>
      </Form>
    </div>
